                    ...
                    <ul class="divide-y divide-gray-100">
                        @forelse($groups as $group)
                            <li class="py-1.5 flex items-center gap-4">
                                ...
                                <div class="flex-1 min-w-0 text-sm text-gray-600 overflow-hidden" style="display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;" title="{{ $group->mappings->map(fn($m) => $m->entity_name ?? $m->entity_slug)->implode(', ') }}">
                                    {{ $group->mappings->map(fn($m) => $m->entity_name ?? $m->entity_slug)->implode(', ') ?: '—' }}
                                </div>
